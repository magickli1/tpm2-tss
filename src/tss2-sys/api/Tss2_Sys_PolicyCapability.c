...
TSS2_RC
Tss2_Sys_PolicyCapability_Prepare(TSS2_SYS_CONTEXT   *sysContext,
    ...
    rval = CommonPreparePrologue(ctx, TPM2_CC_PolicyCapability);
    if (rval)
        return rval;
...
TSS2_RC
Tss2_Sys_PolicyCapability_Complete(TSS2_SYS_CONTEXT *sysContext) {
    TSS2_SYS_CONTEXT_BLOB *ctx = syscontext_cast(sysContext);
...
TSS2_RC
Tss2_Sys_PolicyCapability(TSS2_SYS_CONTEXT             *sysContext,
                          TPMI_SH_POLICY                policySession,
    ...
    rval = Tss2_Sys_PolicyCapability_Prepare(sysContext, policySession, operandB, offset,
                                             operation, capability, property);
    if (rval)
        return rval;
    ...
    return Tss2_Sys_PolicyCapability_Complete(sysContext);
}
