...
static int
test_fapi_key_create_mldsa_sequence(FAPI_CONTEXT *context)
{
    TSS2_RC  r;
    ...
    /* P_MLDSA_SIGN profile: DigestAndSign skips host hash and uses SignSequence. */
    ...
    r = Fapi_VerifySignature(context, "HS/SRK/myMldsaSignKey", message, sizeof(message),
...
test_invoke_fapi(FAPI_CONTEXT *fapi_context)
{
    if (!pqc_esys_tpm_supports_alg(fapi_context->esys, TPM2_ALG_MLDSA)) {
        LOG_INFO("SKIP: Pure ML-DSA FAPI sequence test requires v185 TPM (fwTPM)");
        return EXIT_SKIP;
    }

    return test_fapi_key_create_mldsa_sequence(fapi_context);
}
